...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("Message Freq 2", &fHslider0, FAUSTFLOAT(34.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1e+02f), FAUSTFLOAT(0.001f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		int iSlow7 = int(std::min<float>(fSlow4, float(std::max<int>(0, iSlow6))));
		float fSlow8 = std::floor(fSlow5);
		float fSlow9 = fSlow8 + (1.0f - fSlow5);
		int iSlow10 = int(std::min<float>(fSlow4, float(std::max<int>(0, iSlow6 + 1))));
		float fSlow11 = fSlow5 - fSlow8;
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp4 = fSlow3 * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(int(65536.0f * fRec5[0]), 65535))];
			fVec2[IOTA0 & 1023] = fTemp4;
			fRec3[0] = fSlow11 * fVec2[(IOTA0 - iSlow10) & 1023] + 0.44f * fRec3[1] + fSlow9 * fVec2[(IOTA0 - iSlow7) & 1023] - 0.3f * fRec1[1];
			fRec1[0] = fRec3[1];
